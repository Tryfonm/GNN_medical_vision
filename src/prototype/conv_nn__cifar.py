...
class SimpleCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(-1, 128 * 4 * 4)
        self.embeddings = x.detach().clone() 
...
def train_model(
    ...
    try:
        for epoch in range(epochs):
            ...
            logger.info(
                f"Epoch [{epoch + 1}/{epochs}] Training Loss: {avg_epoch_loss:.3f} | Validation Loss: {val_loss:.3f}"
            )
...
def save_model(model, path="models"):
    ...
    torch.save(
        model.state_dict(),
        os.path.join(
            path, f"{DATETIME_ID}_{SYSTEM_ID}_{os.path.basename(__file__)}.pth"
        ),
    )


def main(epochs=10, lr=0.001, batch_size=64, patience=5):
    ...
    writer = SummaryWriter(comment=f"_{os.path.basename(__file__)}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--lr", default=0.001, type=float, help="training learning rate"
    )
    parser.add_argument(
